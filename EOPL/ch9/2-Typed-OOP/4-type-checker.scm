(module checker (lib "eopl.ss" "eopl")
  ...
  ;****************************************************************
  ; 把问题分解： 如何对一个Class进行type-check呢？======>  ████████ 对于Class的使用者来说：Class不过是个 var/method <--> Type 的集合
  ;   显然，我可以从签名上获取其中各 fields, methods 符号的类型
  ;        那么我要check方法的body-exp (TEnv为fields,self,super)——> 确保其 body-exp的类型与result-type一致

  ; 对于method-call， 我要检查其实参类型 int * bool 与目标method的形式参数一一对应
  ; ===============================================================
  (define (typer src)
  ...
  (define type-of
    (lambda (exp tenv)
      (cases Expression exp
        ...
        ($if-exp (test-exp true-exp false-exp)
                 (let
                     ((test-type (type-of test-exp tenv))
                      (then-type (type-of true-exp tenv))
                      (else-type (type-of false-exp tenv)))
        ...
        ($new-object-exp (cls-name rands)
                         (let ((arg-types (types-of-exps rands tenv))
                               (sc (lookup-static-class cls-name)))
                           (cases StaticClass sc
                             ($an-interface (method-tenv)
                                            (report-cant-instantiate-interface cls-name))
                             ($a-static-class (super-name i-names field-names field-types method-tenv)
                                              ;; check the call to initialize --- new表达式相当于initialize()
                                              (░░type-of-call
                                               (find-method-type cls-name 'initialize) 
                                               arg-types
                                               rands
                                               exp)
    ...
    (lambda (c-decl)
      (cases ClassDecl c-decl
        ...
        ($a-class-decl (class-name super-name i-names field-types field-names method-decls)
                       ; 检查每一个method : 须知道所有相关的types : fields, self, super
                       (let ((sc (lookup-static-class class-name)))
                         (for-each 
                          (lambda (method-decl)
                            (check-method-decl! method-decl class-name super-name (static-class->field-names sc) (static-class->field-types sc)))
                          method-decls))
                       ; 检查该Class是否实现了interface
                       (for-each 
                        (lambda (i-name)
  ...
  ;; check-method-decl! :: MethodDecl * ClassName * ClassName * Listof(FieldName) * Listof(Type)  -> Unspecified
  ;; Page: 368
    ...
    (lambda (m-decl self-name s-name f-names f-types)
      (cases MethodDecl m-decl
        ($a-method-decl (res-type m-name vars var-types body)
                        ; 将fields, args, self/super 这3样的type info加入tenv中 
                        (let ((tenv ($extend-tenv vars var-types
                          ...
                          (let ((body-type (type-of body tenv)))
                            ; 检查body的类型是否 <: res-type
                            (check-is-subtype! body-type res-type m-decl)
                            ; 检查非init方法的类型
                            (if (eqv? m-name 'initialize)
                                #t
                                (let ((maybe-super-type
                                       (maybe-find-method-type (static-class->method-tenv (lookup-static-class s-name)) m-name)))
                                  (if maybe-super-type
  ...
  (define check-if-implements!
    (lambda (c-name i-name)
      (cases StaticClass (lookup-static-class i-name)
