                                ...
                                (cases Answer (██type-of e2 TENV subst1)
                                  ($an-answer (ty2 subst2)                                              
                                              (let [(subst2 (░░unify ty2 (int-type) subst2 e2))] ; T(e2) == Int                                               
                                                ($an-answer (int-type) subst2)))))))) ;  ret Int
      (zero?-exp (e1)
                 (cases Answer (██type-of e1 TENV SUBST)
                   ($an-answer (ty1 subst1)                               
                               (let [(subst2 (░░unify ty1 (int-type) subst1 EXP))] ; T(e1) == Int
                                 ($an-answer (bool-type) subst2))))) ; ret Bool
      (if-exp (e1 e2 e3)
              (cases Answer (██type-of e1 TENV SUBST)
                ($an-answer (ty1 subst)
                              ...
                              (cases Answer (██type-of e2 TENV subst)
                                ($an-answer (ty2 subst)
                                            (cases Answer (██type-of e3 TENV subst)
                                              ($an-answer (ty3 subst)                                                          
                                                          (let [(subst (░░unify ty2 ty3 subst EXP))] ; T(e2) == T(e3) == ret
                                                            ($an-answer ty2 subst))))))))))
      ; ------------------------------------                                   
      (let-exp (var e1 body)
               ...
               (cases Answer (██type-of e1 TENV SUBST)
                 ($an-answer (ty-e1 ss-1)
                             (██type-of body ($extend-tenv var ty-e1 TENV) ss-1))))
      ;; 1-arg proc
                    ...
                    ($an-answer (ty-rator st1)
                                (cases Answer (██type-of rand TENV st1)
                                  ($an-answer (ty-rand st2)         
                                              (let [(st3 (░░unify ty-rator (proc-type ty-rand result-type) st2 EXP))] ; T(rator) = T(rand) -> T(#)
                                                ($an-answer result-type st3))))))))
      ;; letrec ? factorial (x:?) = ... in ...
                    ...
                    (let [(tenv-for-letrec-body ($extend-tenv p-name  (proc-type p-var-type p-result-type) TENV))]
                      (cases Answer (██type-of p-body ($extend-tenv b-var p-var-type  tenv-for-letrec-body) SUBST)
                        ($an-answer (p-body-type subst)
                                    (let ((subst (░░unify p-body-type p-result-type subst p-body))) ; T(p-body) = T(result-type)
